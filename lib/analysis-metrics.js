...
function buildYearDetails(campaigns) {
  ...
  return Array.from(byYear.entries())
    .sort(([left], [right]) => left - right)
    .map(([launchYear, yearCampaigns]) => {
      const completed = yearCampaigns.filter((campaign) =>
      ...
      const averagePledgesUsd = yearCampaigns.map((campaign) => {
      ...
      return {
        launchYear,
        campaignCount: yearCampaigns.length,
        completedCount: completed.length,
        successCount,
        failureCount: completed.length - successCount,
        successRate:
          completed.length > 0
            ? Number(((successCount / completed.length) * 100).toFixed(1))
            : null,
        medianGoalUsd: median(yearCampaigns.map((campaign) => campaign.usd_goal)),
        medianPledgedUsd: median(yearCampaigns.map((campaign) => campaign.usd_pledged)),
        medianBackers: median(yearCampaigns.map((campaign) => campaign.backers_count)),
        medianAveragePledgeUsd: median(averagePledgesUsd),
        medianFundingMultiple: median(
          yearCampaigns.map((campaign) => campaign.funding_multiple),
        ),
        moneyComparableCount: yearCampaigns.filter(
          (campaign) =>
            numeric(campaign.usd_goal) !== null && numeric(campaign.usd_pledged) !== null,
        ).length,
      }
    })
}
